---
title: 'One brain for the whole house: building our home EMS'
date: 2026-08-31T20:00:00+02:00
draft: false
aliases: ['/posts/smart-home-ems-one-arbiter/']
---

Our house has a solar roof, a 10 kWh battery, an EV charger, a heat pump, a hot water tank and a dynamic electricity contract. Each of those came with its own app, and none of those apps know the others exist. Left alone, the EV would happily drain the battery to get "cheaper" solar, the heat pump would fight the battery for the same surplus, and nothing would notice the 15-minute grid peak that drives the Flemish capacity tariff creeping up. So I built one brain for the house: a custom Energy Management System on top of [Home Assistant](https://www.home-assistant.io/) that decides, minute by minute, where the power goes.
...
The core is simple: a priority list for solar surplus. Whenever the roof produces more than the house is using, the surplus goes down a ladder:
...
6. Then dump whatever is left into loads that can take it, like the fridge's power-freeze mode.
...
- Keep it predictable. With the priority list you can always answer why something is or isn't running. Every time I tried to make the logic smarter than that, debugging got harder.
- Real constraints are the best teacher. Fuse limits, tariff structures, inverter limits and a household of end users who file bug reports at breakfast: this is the project I learn most from.
- Quiet failures are the worst. A sensor that stops reporting can switch off a feature without anyone noticing, so a watchdog is next on the list.

The full setup, the hardware that is connected and the software around it, is on the [Smart Home & EMS project page](/projects/smart-home-ems/). The repository stays private, it is our house after all, but if you are building something similar, [send me a mail](mailto:missions@tdlx.nl).
